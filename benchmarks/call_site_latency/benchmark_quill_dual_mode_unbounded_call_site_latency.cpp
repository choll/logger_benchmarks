...
{
  std::remove("benchmark_quill_dual_mode_unbounded_call_site_latency.log");
  ...
  quill::Handler* file_handler =
    quill::file_handler("benchmark_quill_dual_mode_unbounded_call_site_latency.log", "w");
  ...
  {
    run_benchmark("Logger: Quill - Benchmark: Caller Thread Latency, Dual Mode, Unbounded", thread_count,
